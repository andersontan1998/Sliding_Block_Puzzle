...
class Solution{
	...
	bool found;
	void clearBoard(){ //Mark all cells of board empty for further operations
	...
	bool isFound(long int state){ //Check if state is goal state
		int y = state & 7;
		state >>= 3;
		int x = state & 7;
		return x == 3 and y == 1;
	...
	void assignToBoard(long int state){ //Extract state information and mark occupied cells
		clearBoard();
	...
	void generateChildStates(long int state){ //Find all states that can be reached by moving a piece from this state
		long int temp = state;
		for(int i=0;i<NBLOCKS;i++){
			int y = temp & 7;
			temp >>= 3;
	...
	void generateRectangle(long int state,int x, int y, int i){ //Find states that can be reached by moving a rectangle
		//Remove from board
		board[x][y] = false;
		if(B[NBLOCKS-i-1].type == ver){
			if(x+1 < NROWS){
	...
	void tryToPlaceRectangle(long int state,int x,int y,int i){ //Determine if a rectangle can be placed on given coordinates
	...
	void generateSmallSquare(long int state,int x,int y,int i){ //Find states that can be reached by moving a small square
		//Remove from board
		board[x][y] = false;
	...
	void tryToPlaceSmallSquare(long int state,int x,int y,int i){ //Determine if a small square can be placed on given coordinates
		if(x < 0 || x >= NROWS || y < 0 || y >= NCOLS || board[x][y]){
			return;
		}
		long int old_state = state;
	...
	void generateBigSquare(long int state,int x, int y){ //Find states that can be reached by moving big square
		//Remove from board
		for(int i=0;i<2;i++){
			for(int j=0;j<2;j++){
				if(x+i < NROWS && y+j < NCOLS){
	...
	void tryToPlaceBigSquare(long int state,int x, int y){ //Determine if big square can be placed on given coordinates
		for(int i=0;i<2;i++){
			for(int j=0;j<2;j++){
	...
	void assignStateToBlocks(long int state){ //Visualize given state in GUI
		int uw = bframe.w/NCOLS;
		int uh = bframe.h/NROWS;
		for(int i=9;i>=0;i--){
			int y = state & 7;
...
public:
	...
	Solution(){ //Initialize variables that will be used in solve function
		state_index = 0;
		states.clear();
		parentOf.clear();
		visited.clear();
	...
	void solve(){ //Find solution to puzzle using breadth first search algorithm
		state_index = 0;
		Q = queue<long int>();
		states.clear();
		parentOf.clear();
	...
	void next_state(){ //Display next state of puzzle solution in GUI
		if(state_index <= 0){
			return;
		}
		state_index--;
	...
	void previous_state(){ //Display previous state of puzzle solution in GUI
	...
	long int readCurrentState(){ //Read current state of puzzle from GUI
		long int state = 0;
		for(int i=0;i<NBLOCKS;i++){
			state <<= 3;
			state |= B[i].i;
